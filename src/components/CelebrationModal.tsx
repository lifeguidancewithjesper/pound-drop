...
export default function CelebrationModal({ visible, milestone, unit, onClose }: CelebrationModalProps) {
  ...
  const getMessage = () => {
    if (milestone >= 20) return `You're absolutely crushing it! 20+ ${unit} down! 🔥`;
    if (milestone >= 15) return `Incredible progress! 15 ${unit} milestone reached! 💎`;
    if (milestone >= 10) return `Double digits! You lost 10 ${unit}! 🚀`;
    if (milestone >= 5) return `Amazing! 5 ${unit} down! Keep going! 💪`;
    if (milestone >= 2) return `You lost 2 ${unit}! Great start! 🎯`;
